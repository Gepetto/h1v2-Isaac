...
from robot_tasks.utils.history.circular_buffer import CircularBuffer
from robot_tasks.utils.history.manager_term_cfg import ObservationGroupCfg, ObservationTermCfg
...
class ObservationManager(ManagerBase):
    ...
    def compute_group(self, group_name: str) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        for term_name, term_cfg in obs_terms:
            ...
            if term_cfg.noise:
                if hasattr(term_cfg, "model_noise"):
                    obs = term_cfg.model_noise.apply(obs)
                else:
