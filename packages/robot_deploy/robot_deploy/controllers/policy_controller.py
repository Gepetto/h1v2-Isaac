...
class PolicyController:
    ...
    def step(self, state: dict, command: np.ndarray) -> tuple[float, np.ndarray, np.ndarray, np.ndarray]:
        ...
        dt1, q_ref1, kps1, kds1 = self.policies[self.curr_policy_index].step(state, command)
        dt2, q_ref2, kps2, kds2 = self.policies[self.next_policy_index].step(state, command)
        ...
        return min(dt1, dt2), q_ref, kps, kds
